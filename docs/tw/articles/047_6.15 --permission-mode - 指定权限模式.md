# 6.15 --permission-mode - 指定許可權模式

## 概述
`--permission-mode` 標誌允許您指定 Claude Code 的許可權模式，控制工具呼叫的許可權檢查行為。這對於安全性和自動化場景非常重要。
...
模式| 描述| 使用場景
---|---|---
`auto`| 自動批准許可權（預設）| 自動化指令碼、可信環境
`manual`| 手動批准許可權| 互動式開發、需要審查
`always-approve`| 始終批准所有許可權| 完全自動化、高度信任
`always-deny`| 始終拒絕所有許可權| 只讀模式、安全審查
...
```
`--permission-mode` 標誌提供了一種控制工具呼叫許可權的方式。透過選擇不同的許可權模式，您可以：
...
合理使用 `--permission-mode` 可以幫助您在不同場景下獲得最佳的安全性和效率平衡。