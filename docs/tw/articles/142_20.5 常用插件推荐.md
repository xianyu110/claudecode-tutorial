...
```
  * 根據需求描述自動生成程式碼
  * 支援多種程式語言（Python, JavaScript, Java, Go 等）
  * 生成測試程式碼
  * 生成文件註釋
  * 支援程式碼模板定製

**安裝** : claude plugin install code-generator
...
```
  * 支援多種框架（React, Vue, Django, Flask 等）
  * 自定義模板
  * 快速啟動專案
```
  * 包含最佳实践配置 **安装** :
...
```
  * 支援多種測試框架（pytest, unittest, Jest, Mocha 等）
  * 並行測試執行
  * 生成覆蓋率報告
  * CI/CD 整合
```bash
...
專案型別| 推薦外掛
---|---
Web 應用| code-generator, test-runner, api-docs, docker-integration
移動應用| code-generator, test-runner, doc-generator
資料分析| data-transformer, database-connector, doc-generator
微服務| api-generator, docker-integration, cicd-integration, log-monitor
企業應用| security-scanner, audit-logger, performance-monitor

### 根據開發階段選擇
開發階段| 推薦外掛
---|---
專案初始化| boilerplate-generator, readme-generator
開發階段| code-generator, code-formatter, linter, debugger
測試階段| test-runner, mock-generator, coverage-reporter
部署階段| docker-integration, cicd-integration
運維階段| log-monitor, performance-monitor, alert-manager, security-scanner

### 根據團隊規模選擇
團隊規模| 推薦外掛
---|---
個人專案| code-generator, test-runner, doc-generator
小團隊| git-integration, code-formatter, linter, test-runner
中型團隊| cicd-integration, log-monitor, performance-monitor
大型團隊| security-scanner, audit-logger, alert-manager