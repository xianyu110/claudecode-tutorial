...
## 概述
`--include-partial-messages` 標誌允許您在流式響應中包含部分訊息。這對於實時處理、除錯和監控流式輸出非常有用。
...
```
當使用流式輸出時，Claude 的響應會分塊傳送。預設情況下，只有完整的訊息才會被處理。使用 `--include-partial-messages` 標誌後，部分訊息也會被包含在輸出中。
...
```
適合使用 --include-partial-messages 的場景：
