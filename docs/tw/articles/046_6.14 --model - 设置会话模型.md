...
`--model` 標誌允許您指定 Claude Code 使用的模型。不同的模型有不同的效能、速度和成本特點，您可以根據任務需求選擇合適的模型。
...
Claude 是由 Anthropic 開發的最先進的大型語言模型系列。所有當前的 Claude 模型都支援文字和影象輸入、文字輸出、多語言功能和視覺能力。

模型| Claude API ID| 描述| 適用場景| 相對速度| 上下文視窗
---|---|---|---|---|---

Claude Opus 4.5| `claude-opus-4-5-20251101`| 結合最大智慧和實際效能的高階模型| 複雜任務、深度分析、高階程式碼生成| 中等| 200K 令牌
Claude Sonnet 4.5| `claude-sonnet-4-5-20250929`| 用於複雜代理和編碼的智慧模型，提供最佳的智慧、速度和成本平衡| 一般程式設計任務、文件生成、代理任務| 快速| 200K 令牌 / 1M 令牌(測試版)
Claude Haiku 4.5| `claude-haiku-4-5-20251001`| 最快的模型，具有接近前沿的智慧| 快速查詢、簡單任務、快速原型| 最快| 200K 令牌

**別名支援** ：您也可以使用更簡潔的別名 `claude-opus-4-5`、`claude-sonnet-4-5` 和 `claude-haiku-4-5`，這些別名會自動指向最新的模型快照。
...
`--model` 標誌提供了一種選擇合適模型的方式。透過選擇不同的模型，您可以：
...
合理使用 `--model` 可以幫助您在不同場景下獲得最佳的效能和成本效益。
