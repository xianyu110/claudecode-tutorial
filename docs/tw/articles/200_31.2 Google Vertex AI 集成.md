# 31.2 Google Vertex AI 整合

## 31.2.1 Vertex AI 概述
Google Vertex AI 是 Google Cloud 提供的統一機器學習平臺，支援透過 API 訪問多種基礎模型，包括 Anthropic 的 Claude 模型。透過 Vertex AI 使用 Claude Code 可以為企業帶來以下優勢：

### Vertex AI 的優勢
  1. **GCP 原生整合** ：與 Google Cloud IAM、Cloud Audit Logs、Cloud Monitoring 等服務無縫整合
  2. **全球端點** ：支援全球訪問，提供更好的延遲和可用性
  3. **100 萬令牌上下文視窗** ：支援超長上下文，適合大型程式碼庫分析
  4. **企業級安全** ：符合 Google Cloud 安全標準和合規要求
  5. **靈活的部署** ：支援區域和全域性端點，滿足不同需求

### 適用場景
  * 已經使用 Google Cloud Platform 的企業
  * 需要超長上下文視窗的應用
  * 要求使用 Google Cloud IAM 進行身份驗證的場景
  * 需要全球訪問和低延遲的環境
...
```
透過正確配置 Google Vertex AI，企業可以利用 Google Cloud 的強大基礎設施，安全、高效地部署 Claude Code，並享受超長上下文視窗帶來的優勢。