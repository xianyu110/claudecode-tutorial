# 31.1 Amazon Bedrock 整合

## 31.1.1 Bedrock 概述

Amazon Bedrock 是 AWS 提供的完全託管服務，可以透過 API 訪問基礎模型，包括 Anthropic 的 Claude 模型。透過 Bedrock 使用 Claude Code 可以為企業帶來以下優勢：

### Bedrock 的優勢

  1. **AWS 原生整合** ：與 AWS IAM、CloudTrail、CloudWatch 等服務無縫整合
  2. **企業級安全** ：符合 AWS 安全標準和合規要求
  3. **靈活的部署** ：支援多個 AWS 區域，滿足資料駐留要求
  4. **成本管理** ：透過 AWS Cost Explorer 和 Budgets 進行成本控制
  5. **高可用性** ：利用 AWS 的全球基礎設施和冗餘機制
...
透過正確配置 Amazon Bedrock，企業可以利用 AWS 的強大基礎設施，安全、高效地部署 Claude Code。
